// app/api/projects/create-from-snapshot/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate payload size to prevent abuse (limit to ~500KB)
    ...
    console.log('Generating deep analysis for:', snapshot.product_name, 'User:', user.id);
    ...
    // Create project - START WITH BASIC DATA
    // We'll store ai_analysis separately if column doesn't exist yet
    ...
    try {
      projectData.playbook_snapshot = snapshot;
      projectData.ai_analysis = aiAnalysis;
    } catch (e) {
      console.log('New columns not available yet, will skip for now');
    }
    ...
    // Store analysis in localStorage as backup until migration is run
    if (typeof window !== 'undefined') {
      localStorage.setItem(`project_${project.id}_analysis`, JSON.stringify(aiAnalysis));
      localStorage.setItem(`project_${project.id}_snapshot`, JSON.stringify(snapshot));
    }

    return NextResponse.json({
      projectId: project.id,
      project,
      note: 'AI analysis stored in localStorage. Run database migration to persist in database.'
    }, { status: 200 });

  } catch (error: any) {
    console.error('Error in create-from-snapshot:', error);
    return NextResponse.json(
